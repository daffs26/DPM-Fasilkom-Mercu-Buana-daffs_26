    ...
    <>
      ...
      <div className="hidden lg:grid grid-cols-4 gap-4">
        ...
        <Card className="rounded-3xl p-5 border-slate-200/80 shadow-soft flex flex-col justify-between min-h-[160px]">
          ...
          <div className="flex items-center justify-between text-xs pt-2.5 border-t border-slate-100">
            <span className="text-slate-500 text-[11px]">Batas Waktu Pengajuan</span>
            <span className={`text-[11px] font-bold ${
              filteredProkers.filter(p => p.proposal?.isDadakan).length > 0 ? 'text-amber-600 font-extrabold' : 'text-slate-600'
            }`}>
              {filteredProkers.filter(p => p.proposal?.isDadakan).length > 0 
        ...
        <Card className="rounded-3xl p-5 border-slate-200/80 shadow-soft flex flex-col justify-between min-h-[160px]">
          ...
          <div className="flex items-center justify-between text-xs pt-2.5 border-t border-slate-100">
            <span className="text-slate-500 text-[11px]">Tingkat Penyelesaian</span>
            <span className="text-emerald-600 font-bold text-[11px]">
              {totalProkers > 0 ? Math.round((completedProkers / totalProkers) * 100) : 0}% Capaian
            </span>
          </div>
        </Card>

        {/* Card 4: Keterlambatan LPJ (SLA Breach) */}
        <Card className={`rounded-3xl p-5 shadow-soft flex flex-col justify-between min-h-[160px] ${
